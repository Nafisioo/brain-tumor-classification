...
def process_gradcam_images():
    """Stacks the two Grad-CAM comparison figures into one panel, rendered with the same matplotlib/300 DPI pipeline as the other assets."""  # noqa: E501
...
def draw_flow_diagram(steps, output_path, highlights=[]):
    ...
    total_height = (
        len(steps) * box_height + (len(steps) - 1) * arrow_height + (padding * 2)
    )
    ...
    for i, step in enumerate(steps):
        ...
        draw.text(
            (center_x - (text_w // 2), top + (box_height - text_h) // 2 - 2),
            step,
            fill=current_text,
            font=font,
        )
        ...
        if i < len(steps) - 1:
            ...
            draw.line(
                [(center_x, y_cursor + 4), (center_x, arr_end)],
                fill=accent_color,
                width=2,
            )
            draw.polygon(
                [
                    (center_x - 6, arr_end - 6),
                    (center_x + 6, arr_end - 6),
                    (center_x, arr_end),
                ],
                fill=accent_color,
...
def generate_all_diagrams():
    ...
    draw_flow_diagram(
        layer_stack, "assets/cnn_v2.png", highlights=["224×224", "4 Classes"]
    )
    draw_flow_diagram(
        layer_stack, "assets/resnet18.png", highlights=["224×224", "4 Classes"]
    )
